...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className="w-[280px] bg-sidebar border-r border-sidebar-border flex flex-col">
      <div className="p-4 flex items-center justify-between metal-panel border-b border-sidebar-border">
        <div className="flex items-center gap-2">
          <span className="text-lg">🚉</span>
          <div>
            <h2 className="text-base font-heading font-bold text-foreground leading-none">AI STATION</h2>
            <span className="text-[9px] font-station text-station-gold tracking-widest">CENTRAL TERMINAL</span>
          </div>
        </div>
        <button onClick={() => setCollapsed(true)} className="p-1 text-sidebar-foreground hover:text-foreground hover-lift">
          <ChevronLeft size={18} />
        </button>
      </div>

      {/* Marching rail strip under header */}
      <div className="h-1 rail-ties opacity-60" />
      ...
      <div className="flex-1 overflow-y-auto px-3 scrollbar-thin pt-2">
        {sessions.length > 0 && (
          <div className="px-2 pb-1 text-[9px] font-station text-station-cyan tracking-widest">
            ◇ JOURNEY LOG
          </div>
        )}
        {sessions.map((s, i) => (
          <div
            ...
            className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left mb-1 group transition-all cursor-pointer animate-train-arrive border ${
              sessionId === s.id
                ? 'bg-sidebar-accent text-foreground border-station-cyan/50 shadow-[inset_2px_0_0_hsl(var(--station-cyan))]'
                : 'text-sidebar-foreground hover:bg-sidebar-accent/50 border-transparent hover:border-station-border'
            }`}
          >
